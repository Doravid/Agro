...
void Clay_Raylib_Render(Clay_RenderCommandArray renderCommands, Font *fonts) {
    for (int j = 0; j < renderCommands.length; j++) {
        ...
        Clay_BoundingBox boundingBox = {renderCommand->boundingBox.x,
        ...
        case CLAY_RENDER_COMMAND_TYPE_BORDER: {
            ...
            if (config->width.right > 0) {
                DrawRectangleV(
                    (Vector2){boundingBox.x + boundingBox.width -
                                  config->width.right,
            ...
            if (config->width.top > 0) {
                DrawRectangleV(
                    (Vector2){boundingBox.x + config->cornerRadius.topLeft,
                              boundingBox.y},
                    (Vector2){boundingBox.width - config->cornerRadius.topLeft -
                                  config->cornerRadius.topRight,
